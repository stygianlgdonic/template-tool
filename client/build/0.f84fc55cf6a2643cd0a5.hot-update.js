...
const UCircle = ({
  ...
  const [image] = use_image__WEBPACK_IMPORTED_MODULE_2___default()(patternImageUrl || null); // const circleWidth = shapeRef.current?.getClientRect().width;
  // const circleHeight = shapeRef.current?.getClientRect().height;

  const imageWidth = image ? image.width : 10;
  const imageHeight = image ? image.height : 10; // testing

  const Xpoint = imageWidth / 2;
  const Ypoint = imageHeight / 2; // const patternScale = restProps.patternScale || Math.max(
  //     circleWidth / imageWidth,
  //     circleHeight / imageHeight
  // );
  // console.log({ Xpoint, Ypoint, circleHeight, circleWidth, imageHeight, imageWidth })
...
//# sourceMappingURL=0.f84fc55cf6a2643cd0a5.hot-update.js.map