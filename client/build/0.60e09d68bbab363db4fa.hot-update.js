...
const MainStage = ({
  ...
  const getLineGuideStops = skipShape => {
    ...
    $stage.current.find(".object").forEach(guideItem => {
      ...
      const trNodes = $tr === null || $tr === void 0 ? void 0 : (_$tr$current = $tr.current) === null || _$tr$current === void 0 ? void 0 : _$tr$current.nodes();
      const isSelected = trNodes === null || trNodes === void 0 ? void 0 : trNodes.some(node => {
        console.log({
          node: node.attrs.id,
          item: guideItem.attrs.id
        });
        return node.attrs.id === guideItem.attrs.id;
      }); // console.log({ isSelected: $tr?.current })
  ...
  const _onDragMove = e => {
    ...
    const itemBounds = getObjectSnappingEdges($tr.current); // const itemBounds = getObjectSnappingEdges(e.target);
...
//# sourceMappingURL=0.60e09d68bbab363db4fa.hot-update.js.map