...
const CardElementsFunctions = () => {
  ...
  const handleTextTransform = type => {
    setCardData(prev => {
      const shapeIndex = prev.elements.findIndex(item => item.id === selectedId);
      prev.elements[shapeIndex].textTransform = _utils_defaults__WEBPACK_IMPORTED_MODULE_4__["fontSizeArray"].find(item => item.name === type).type;
    });
  };
...
//# sourceMappingURL=0.c55373e7bd234f63a4dd.hot-update.js.map